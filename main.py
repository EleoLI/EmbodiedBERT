...
#load sensorimotor regressors
auditory = torch.load('auditory_mapping_regressor_model.pth')
gustatory = torch.load('gustatory_mapping_regressor_model.pth')
haptic = torch.load('haptic_mapping_regressor_model.pth')
interoceptive = torch.load('interoceptive_mapping_regressor_model.pth')
olfactory = torch.load('olfactory_mapping_regressor_model.pth')
visual = torch.load('visual_mapping_regressor_model.pth')
foot_leg = torch.load('foot_leg_mapping_regressor_model.pth')
hand_arm = torch.load('hand_arm_mapping_regressor_model.pth')
head = torch.load('head_mapping_regressor_model.pth')
mouth = torch.load('mouth_mapping_regressor_model.pth')
torso = torch.load('torso_mapping_regressor_model.pth')
...
def main():
    ...
    if "saves" in args.bert_model:
        ...
        log_dir = make_log_dir(os.path.join("saves", args.model_type))
        logger = Logger(log_dir)
        config.save(log_dir)
    ...
    # build tokenizer and model and regressor
    ...
    ########### Inference ###########
    # VUA18 / VUA20
    if  args.do_test and task_name == "vua":
        # if test data is genre or POS tag data
        cols = ['target','accuracy','precision','recall','f1']
        Evaluation = pd.DataFrame(columns = cols)
        cols2 = ['preds', 'out_label_ids', 'pred_guids', 'Context_sm', 'Basic_sm']
        if ("genre" in args.data_dir) or ("pos" in args.data_dir):
            ...
            for idx, target in tqdm(enumerate(targets)):
                SM_values = pd.DataFrame(columns = cols2)
                ...
                if args.return_sm:
                    result, preds, out_label_ids, pred_guids, Context_sm, Basic_sm = run_eval(args, logger, model, eval_dataloader, all_guids, task_name, return_preds=True)
                    SM_values['preds'] = preds
                    SM_values['pred_guids'] = pred_guids[0]
                    SM_values['out_label_ids'] = out_label_ids
                    SM_values['Context_sm'] = Context_sm
                    SM_values['Basic_sm'] = Basic_sm
                    #SM_values.to_csv('pos_sm/' +  str(args.seed) + target + '_result.csv')
                    
                else:
                    results = run_eval(args, logger, model, eval_dataloader, all_guids, task_name)
                    accuracy = results['acc']
                    precision = results['precision']
                    recall = results['recall']
                    f1 = results['f1']
                    Evaluation.loc[len(Evaluation)] = [target,accuracy,precision,recall,f1]
                    #Evaluation.to_csv('genre_testing/' +  str(args.seed) + target + '_result.csv')
            ...
            if args.return_sm:
                ...
                SM_values['Context_sm'] = Context_sm
                SM_values['Basic_sm'] = Basic_sm
                #SM_values.to_csv('vua20_sm/' +  str(args.seed)  + '_result.csv')
            else:
                results = run_eval(args, logger, model, eval_dataloader, all_guids, task_name)
                accuracy = results['acc']
                precision = results['precision']
                recall = results['recall']
                f1 = results['f1']
                Evaluation.loc[len(Evaluation)] = [accuracy,precision,recall,f1]
                #Evaluation.to_csv('VUAverb_testing/' +  str(args.seed) + '_' + str(args.train_batch_size) + '_' + str(args.learning_rate) + '_result.csv')

    # TroFi / MOH-X (K-fold)
    elif args.do_test and args.task_name == "trofi":
        logger.info(f"***** Testing with {args.data_dir}")
        k_result = []
        ...
        cols = ['accuracy','precision','recall','f1']
        Evaluation = pd.DataFrame(columns=cols)
        accuracy = avg_result['acc']
        precision =avg_result['precision']
        recall =avg_result['recall']
        f1 = avg_result['f1']
        Evaluation.loc[len(Evaluation)] = [accuracy,precision,recall,f1]
        #Evaluation.to_csv('TROFI_testing/' +  str(args.seed)  +'_result.csv')
...
def run_train(
    ...
    for epoch in trange(int(args.num_train_epoch), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if args.model_type in ["MELBERT_MIP", "MELBERT","EmbodiedBERT_MIP_2"]:
                (
                    input_ids,
                    input_mask,
                    segment_ids,
            ...
            if args.model_type in ["MELBERT_MIP", "MELBERT","EmbodiedBERT_MIP_2"]:
                logits, context_sm, basic_sm = model(
                    input_ids,
                    input_ids_2,
                    target_mask=(segment_ids == 1),
        ...
        if args.do_eval:
            ...
            result = run_eval(args, logger, model, eval_dataloader, all_guids, task_name,return_preds=False)
...
def run_eval(args, logger, model, eval_dataloader, all_guids, task_name,return_preds=False):
    ...
    for eval_batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if args.model_type in ["MELBERT_MIP", "MELBERT","EmbodiedBERT_MIP_2"]:
                ...
                for i in context_sm:
                    Context_sm.append(i.detach().cpu().numpy())
                for i in basic_sm:
                    Basic_sm.append(i.detach().cpu().numpy())
    ...
    if return_preds:
        return result, preds, out_label_ids, pred_guids, Context_sm, Basic_sm
    
    # if return_preds:
    #     return preds, out_label_ids, pred_guids[0]
...
def load_pretrained_model(args):
    ...
    regressors = [auditory, gustatory, haptic, interoceptive, olfactory, visual,
    ...
    if args.model_type == "EmbodiedBERT_MIP_2":
        model = EmbodiedBERT_MIP_2(
            args=args, Model=bert, config=config, regressors= regressors,num_labels=args.num_labels
        )
