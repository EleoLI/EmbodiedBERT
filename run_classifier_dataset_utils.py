...
def convert_examples_to_two_features_basic(examples, label_list, max_seq_length, tokenizer, output_mode, args):
    """Loads a data file into a list of `InputBatch`s."""
    label_map = {label: i for i, label in enumerate(label_list)}
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        try:
            if POS == 'NOUN':
                POS = 'n'
                basic_sen, basic_idx = basicer(target,POS)
            ...
            if POS == 'ADV':
                ...
                basic_sen, basic_idx = basicer(target,None)
    ...
    for token in doc:
        if token.text == word: 
            ...
            for i in target.children:
                if i.pos_ == 'PUNCT':
                    continue
                if i.dep_ in {"aux", "auxpass"} and token.lemma_ in {"can", "could", "may", 
                                                                         "might", "must", "shall", "should", "will", "would"}:
                    continue
                if i.text == 'n\'t':
                    continue
                if i.pos_ == 'NOUN' or 'VERB' or 'ADJ' or 'ADV':                    
                    deps.append(i)                    
            deps.append(target.head)
            if target in deps:
                deps.remove(target)
            break            
    return deps
